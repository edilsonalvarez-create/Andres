/* ============================================================================
   QA Guardian — Esquema Hangfire (SQL Server)
   Sprint 20-A (B5): Install.sql oficial de Hangfire.SqlServer 1.8.18
   (https://github.com/HangfireIO/Hangfire/blob/v1.8.18/src/Hangfire.SqlServer/Install.sql).
   Target schema version = 9. Idempotente: re-ejecutar no recrea objetos ya presentes.

   Ejecutar con cuenta privilegiada (sa / DBA) DESPUÉS de migrate / esquema EF y
   ANTES de arrancar el API. El API usa PrepareSchemaIfNecessary=false en prod.

   Uso (sqlcmd):
     sqlcmd -S <server> -U sa -P "<sa-password>" -C -d QAGuardian \
       -v HangFireSchema="HangFire" \
       -i database/05-hangfire-schema.sql
   ============================================================================ */

:setvar HangFireSchema "HangFire"

USE QAGuardian;
GO

/* sqlcmd deja QUOTED_IDENTIFIER OFF por defecto; Hangfire.SqlServer (Install.sql
   vía SqlServerObjectsInstaller) lo activa. Sin esto, los índices filtrados
   de schema v6+ fallan con Msg 1934 y el esquema queda a medias. */
