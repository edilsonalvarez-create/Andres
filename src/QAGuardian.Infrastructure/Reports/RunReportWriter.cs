...
/// <summary>
/// Genera el contenido binario de un reporte de ejecución (<see cref="TestRun"/>) en los 8
/// formatos soportados. Extraído de <see cref="RunReportGenerator"/> (Sprint 4, Extract Class):
/// esa clase concentraba 3 familias de reportes no relacionadas entre sí (run, dashboard,
/// matriz de ejecución) violando SRP — ver ADR-008.
/// </summary>
public class RunReportWriter
{
    public byte[] Write(TestRun run, string projectName, ReportFormat format) => format switch
    {
        ReportFormat.Json => GenerateJson(run, projectName),
        ReportFormat.Csv => GenerateCsv(run),
        ReportFormat.Html => GenerateHtml(run, projectName),
        ReportFormat.Excel => GenerateExcel(run, projectName),
        ReportFormat.Pdf => GeneratePdf(run, projectName),
        ReportFormat.Word => GenerateWord(run, projectName),
        ReportFormat.Xml => GenerateXml(run, projectName),
