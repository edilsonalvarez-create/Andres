...
public class CatalogHandlerTests
{
    ...
    public async Task Crear_requerimiento_valido()
    {
        ...
        var handler = new CreateRequirementCommandHandler(modules, requirements, _uow);
    ...
    public async Task Crear_historia_valida()
    {
        ...
        requirements.GetByIdAsync(reqId, Arg.Any<CancellationToken>())
            .Returns(new Requirement(Guid.NewGuid(), "REQ-1", "T", null));

        var handler = new CreateUserStoryCommandHandler(requirements, stories, _uow);
        var result = await handler.Handle(
    ...
    public async Task Crear_version_duplicada_falla()
    {
        ...
        var handler = new CreateProjectVersionCommandHandler(projects, versions, _uow);
    ...
    public async Task Marcar_version_como_liberada()
    {
        ...
        var handler = new ReleaseProjectVersionCommandHandler(versions, _uow);
